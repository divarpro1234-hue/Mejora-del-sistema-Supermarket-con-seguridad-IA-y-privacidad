...
@Injectable()
export class SupermarketService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(SupermarketService.name);
  private backupTimer: NodeJS.Timeout | null = null;
  private backupEnEjecucion = false;

  constructor(private readonly databaseService: DatabaseService) {}

  async onModuleInit() {
    await this.inicializarBackupsAutomaticosSeguro();
  }
  ...
  private async inicializarBackupsAutomaticosSeguro() {
    try {
      await this.asegurarTablaBackupAutomatico();

      this.backupTimer = setInterval(() => {
        void this.evaluarBackupAutomaticoProgramado();
      }, 60_000);
      ...
      this.logger.warn(`No se pudo inicializar el programador de backups automaticos: ${this.obtenerMensajeErrorBackup(error)}`);
    }
  }
  ...
  private obtenerFrecuenciasBackupPermitidas() {
    return [
      { horas: 1, etiqueta: 'Cada 1 hora', descripcion: 'Alta frecuencia para pruebas y operación crítica.' },
      { horas: 4, etiqueta: 'Cada 4 horas', descripcion: 'Protección frecuente sin saturar el equipo.' },
      { horas: 7, etiqueta: 'Cada 7 horas', descripcion: 'Frecuencia intermedia solicitada para operación diaria.' },
      { horas: 9, etiqueta: 'Cada 9 horas', descripcion: 'Opción flexible para turnos largos.' },
      { horas: 12, etiqueta: 'Cada 12 horas', descripcion: 'Dos copias al día.' },
      { horas: 24, etiqueta: 'Cada 24 horas', descripcion: 'Copia diaria estándar.' },
    ];
  }

  private validarFrecuenciaBackup(frecuenciaHoras: any) {
    const horas = Number(frecuenciaHoras);
    const permitidas = this.obtenerFrecuenciasBackupPermitidas().map((item) => item.horas);

    if (!Number.isInteger(horas) || !permitidas.includes(horas)) {
      throw new Error(`Frecuencia no permitida. Usa una de estas opciones: ${permitidas.join(', ')} horas.`);
    }

    return horas;
  }

  private obtenerMensajeErrorBackup(error: any) {
    return String(error?.message || error?.stderr || error || 'Error desconocido');
  }

  private localizarPgDump() {
    const candidatos = [
    ...
    const candidatoExistente = candidatos.find((candidato) => candidato && candidato !== 'pg_dump' && existsSync(candidato));
    return candidatoExistente || 'pg_dump';
  }

  private obtenerDirectorioBackups() {
    const directorio = process.env.BACKUP_DIR || join(process.cwd(), 'backups');
  ...
  private formatearFechaArchivoBackup(fecha = new Date()) {
    return fecha
      .toISOString()
      .replace(/:/g, '-')
      .replace(/\..+$/, '')
  ...
  private async asegurarTablaBackupAutomatico() {
    await this.databaseService.query(`
  ...
  async obtenerBackupAutomaticoConfiguracion() {
    await this.asegurarTablaBackupAutomatico();

    const configuracion = await this.databaseService.query(`
      SELECT
        id_configuracion,
        activo,
        frecuencia_horas,
        CASE
          WHEN frecuencia_horas = 1 THEN 'Cada 1 hora'
          ELSE 'Cada ' || frecuencia_horas || ' horas'
        ...
        updated_at,
        CASE
          WHEN activo = false THEN 'DESACTIVADO'
          WHEN proxima_ejecucion IS NULL THEN 'SIN_PROGRAMAR'
          WHEN proxima_ejecucion <= now() THEN 'LISTO_PARA_EJECUTAR'
          ELSE 'PROGRAMADO'
        END AS estado_programacion
      FROM ops.backup_configuracion_automatica
      WHERE id_configuracion = 1
    `);

    return {
      ...(configuracion[0] || {}),
      backup_en_ejecucion: this.backupEnEjecucion,
      opciones: this.obtenerFrecuenciasBackupPermitidas(),
      nota: 'El programador vive en el backend NestJS. Si el backend se apaga, las copias automaticas se pausan y continuan al volver a iniciar.',
    };
  }

  async actualizarBackupAutomaticoConfiguracion(
    frecuenciaHoras: any,
    activo: any = true,
    usuario = 'admin',
  ) {
    ...
    const horas = this.validarFrecuenciaBackup(frecuenciaHoras);
    const activoBooleano = activo === true || String(activo).toLowerCase() === 'true';
      ...
      `
        ...
        SET activo = $1,
            ...
            proxima_ejecucion = CASE
              WHEN $1::boolean THEN now() + ($2::int * interval '1 hour')
              ELSE NULL
            END,
            estado_ultima_ejecucion = CASE
              WHEN $1::boolean THEN 'PROGRAMADO'
              ELSE 'DESACTIVADO'
            END,
            ultimo_mensaje = CASE
              WHEN $1::boolean THEN 'Configuracion automatica actualizada desde el panel.'
              ELSE 'Backups automaticos desactivados desde el panel.'
            END,
            actualizado_por = $3,
            updated_at = now()
        WHERE id_configuracion = 1
      `,
      [activoBooleano, horas, usuario || 'admin'],
    );

    return this.obtenerBackupAutomaticoConfiguracion();
  }

  async ejecutarBackupAhora(usuario = 'admin') {
    const resultado = await this.ejecutarBackupServidor('MANUAL', usuario || 'admin');
    const configuracion = await this.obtenerBackupAutomaticoConfiguracion();
  ...
  private async evaluarBackupAutomaticoProgramado() {
    if (this.backupEnEjecucion) return;
    ...
    try {
      ...
      await this.ejecutarBackupServidor('AUTOMATICO', 'SISTEMA_BACKUP_AUTO');
    } catch (error) {
      this.logger.warn(`No se pudo evaluar el backup automatico: ${this.obtenerMensajeErrorBackup(error)}`);
    }
  }

  private async ejecutarBackupServidor(origen: 'MANUAL' | 'AUTOMATICO', usuario: string) {
    if (this.backupEnEjecucion) {
      return {
        ok: false,
        estado: 'EN_PROCESO',
        mensaje: 'Ya existe una copia de seguridad en ejecucion. Espera a que finalice.',
      };
    }

    this.backupEnEjecucion = true;
    let idBackup: string | null = null;
    let archivoDestino = '';
    ...
    try {
      ...
      await this.databaseService.query(`
        UPDATE ops.backup_configuracion_automatica
        SET estado_ultima_ejecucion = 'EN_PROCESO',
            ultimo_mensaje = 'Backup en proceso desde backend NestJS.',
            actualizado_por = $1,
            updated_at = now()
        WHERE id_configuracion = 1
      `, [usuario]);

      const politica = await this.databaseService.query(`
        SELECT id_politica
      ...
      const idPolitica = politica[0]?.id_politica || null;
      const dbName = process.env.DB_NAME || process.env.DB_DATABASE || 'supermarket_db';
      const dbUser = process.env.DB_USER || process.env.DB_USERNAME || 'postgres';
      const dbHost = process.env.DB_HOST || 'localhost';
      const dbPort = process.env.DB_PORT || '5432';
      const dbPassword = process.env.DB_PASSWORD || '';
      const directorio = this.obtenerDirectorioBackups();
      const nombreArchivo = `${dbName}_backup_${this.formatearFechaArchivoBackup()}.sql`;
      archivoDestino = join(directorio, nombreArchivo);

      const inicio = await this.databaseService.query(
        `SELECT ops.registrar_backup_inicio('COMPLETO', $1, $2) AS id_backup`,
        [archivoDestino, idPolitica],
      );

      idBackup = String(inicio[0]?.id_backup || '');

      const pgDump = this.localizarPgDump();
      const argumentos = [
        '-h', dbHost,
        '-p', String(dbPort),
        '-U', dbUser,
      ...
      await execFileAsync(pgDump, argumentos, {
        env: {
          ...process.env,
          PGPASSWORD: dbPassword,
        },
        windowsHide: true,
        timeout: Number(process.env.BACKUP_TIMEOUT_MS || 180000),
        maxBuffer: 1024 * 1024 * 10,
      });

      const contenido = readFileSync(archivoDestino);
      const hash = createHash('sha256').update(contenido).digest('hex');
      const tamano = statSync(archivoDestino).size;

      await this.databaseService.query(
        `SELECT ops.registrar_backup_fin($1, true, $2, $3, $4)`,
        [idBackup, hash, tamano, `Backup ${origen.toLowerCase()} ejecutado correctamente desde NestJS.`],
      );
      ...
      await this.databaseService.query(`
        ...
        SET ultima_ejecucion = now(),
            proxima_ejecucion = CASE
              WHEN activo THEN now() + (frecuencia_horas * interval '1 hour')
              ELSE NULL
            END,
            ...
            updated_at = now()
        WHERE id_configuracion = 1
      `, [idBackup, archivoDestino, `Backup ${origen.toLowerCase()} ejecutado correctamente.`, usuario]);
      ...
      return {
        ...
        hash_sha256: hash,
        tamano_bytes: tamano,
        mensaje: `Backup ${origen.toLowerCase()} ejecutado correctamente.`,
      };
    } catch (error) {
      const mensaje = this.obtenerMensajeErrorBackup(error);

      if (idBackup) {
        await this.databaseService.query(
          `SELECT ops.registrar_backup_fin($1, false, NULL, NULL, $2)`,
          [idBackup, mensaje],
        );
      }
      ...
      await this.databaseService.query(`
        ...
        SET ultima_ejecucion = now(),
            proxima_ejecucion = CASE
              ...
              ELSE NULL
            END,
            estado_ultima_ejecucion = 'FALLIDO',
            ultimo_id_backup = $1,
            ultimo_archivo = $2,
            ultimo_mensaje = $3,
            actualizado_por = $4,
            updated_at = now()
        WHERE id_configuracion = 1
      `, [idBackup, archivoDestino || null, mensaje, usuario]);

      return {
        ok: false,
        estado: 'FALLIDO',
        id_backup: idBackup,
        archivo: archivoDestino || null,
        mensaje,
      };
    } finally {
      this.backupEnEjecucion = false;
    }
  }
  ...
  private normalizarPagoOperacion(metodoPago: string, pagoAdicional: any = {}) {
    const metodo = String(metodoPago || 'EFECTIVO').toUpperCase();
    const referencia = pagoAdicional?.referenciaPago
      ? String(pagoAdicional.referenciaPago).slice(0, 100)
      : null;
    const tarjeta = pagoAdicional?.tarjetaEnmascarada
      ? String(pagoAdicional.tarjetaEnmascarada).slice(0, 30)
      : null;
    const autorizacion = pagoAdicional?.codigoAutorizacion
      ? String(pagoAdicional.codigoAutorizacion).slice(0, 80)
      : null;
    const monto = Number(pagoAdicional?.montoPago || 0);
    const estadoDefault =
      metodo === 'TARJETA'
        ? 'AUTORIZADO'
        : metodo === 'EFECTIVO'
    ...
    const estado = pagoAdicional?.estadoPago
      ? String(pagoAdicional.estadoPago).slice(0, 30)
      : estadoDefault;
    ...
    const metadata = {
      ...
      estadoPago: estado,
      montoPago: Number.isFinite(monto) ? monto : 0,
      tarjetaEnmascarada: tarjeta,
      codigoAutorizacion: autorizacion,
      fechaVencimientoPago: pagoAdicional?.fechaVencimientoPago || null,
      qrPayload: pagoAdicional?.qrPayload || null,
      nota:
        metodo === 'TARJETA'
          ? 'Pago con tarjeta ficticia. No se almacena el numero completo ni CVV.'
          : metodo === 'QR'
            ? 'Pago QR simulado con monto dinamico del carrito.'
            : metodo === 'TRANSFERENCIA'
              ? 'Pago por transferencia con codigo CPT interno.'
              : 'Pago en efectivo.',
    };
    ...
    const pagoNormalizado = this.normalizarPagoOperacion(metodoPago, pagoAdicional);
    ...
    await this.databaseService.query(
      ...
      SET
        referencia_pago = $2,
        estado_pago = $3,
        tarjeta_enmascarada = $4,
        codigo_autorizacion = $5,
        pago_metadata = $6::jsonb
      WHERE id_venta = $1
      `,
      [
        idVenta,
        pagoNormalizado.referenciaPago,
        pagoNormalizado.estadoPago,
        pagoNormalizado.tarjetaEnmascarada,
        pagoNormalizado.codigoAutorizacion,
        JSON.stringify(pagoNormalizado.metadata),
      ],
    );
  ...
  async registrarPedidoVirtual(
    ...
    const pagoNormalizado = this.normalizarPagoOperacion(
      pagoAdicional?.metodoPago || 'EFECTIVO',
      pagoAdicional,
    );
    ...
    return this.databaseService.query(
      ...
      pedido AS (
        ...
        SELECT
          ...
          $7,
          $8,
          COALESCE(NULLIF($9::numeric, 0), r.total),
          $10,
          $11,
      ...
      [
        ...
        sucursal,
        JSON.stringify(items || []),
        pagoNormalizado.metodo,
        pagoNormalizado.referenciaPago,
        pagoNormalizado.estadoPago,
        pagoNormalizado.montoPago,
        pagoNormalizado.tarjetaEnmascarada,
        pagoNormalizado.codigoAutorizacion,
        pagoNormalizado.fechaVencimientoPago || '',
        JSON.stringify(pagoNormalizado.qrPayload || pagoNormalizado.metadata || {}),
      ],
    );
  }
  ...
  async confirmarPedidoVirtual(
    ...
    const cajaEncontrada = await this.databaseService.query(
  `
  SELECT
    c.id_caja
  FROM app.cajas c
  INNER JOIN app.sucursales s ON s.id_sucursal = c.id_sucursal
  WHERE s.nombre = $1
    AND c.activo = true
...
if (!cajaEncontrada || cajaEncontrada.length === 0) {
  return [
    {
      exito: false,
      mensaje: `No existe una caja activa para la sucursal ${pedido.sucursal}.`,
      pedido,
    },
  ];
}

const idCaja = Number(cajaEncontrada[0].id_caja);
    ...
    const pagoFinal = this.normalizarPagoOperacion(
      metodoPago || pedido.metodo_pago || 'EFECTIVO',
      {
        referenciaPago: pagoAdicional?.referenciaPago || pedido.referencia_pago,
        estadoPago: 'CONFIRMADO',
        montoPago: pagoAdicional?.montoPago || pedido.monto_pago || pedido.total_estimado,
        tarjetaEnmascarada: pagoAdicional?.tarjetaEnmascarada || pedido.tarjeta_enmascarada,
        codigoAutorizacion: pagoAdicional?.codigoAutorizacion || pedido.codigo_autorizacion,
        fechaVencimientoPago:
          pagoAdicional?.fechaVencimientoPago || pedido.fecha_vencimiento_pago,
        qrPayload: pagoAdicional?.qrPayload || pedido.qr_payload,
      },
    );
    ...
    await this.databaseService.query(
      ...
      [
        ...
        venta[0].id_venta,
        pagoFinal.metodo,
        pagoFinal.referenciaPago,
        pagoFinal.montoPago,
        pagoFinal.tarjetaEnmascarada,
        pagoFinal.codigoAutorizacion,
      ],
    );
    ...
    return [
      {
        ...
        pedido: {
          ...
          metodo_pago: pagoFinal.metodo,
          referencia_pago: pagoFinal.referenciaPago,
          estado_pago: 'CONFIRMADO',
          monto_pago: pagoFinal.montoPago,
          tarjeta_enmascarada: pagoFinal.tarjetaEnmascarada,
          codigo_autorizacion: pagoFinal.codigoAutorizacion,
        },
        venta: venta[0],
      },
    ];
  ...
  private contarCoincidenciasIA(texto: string, palabras: string[]) {
    const textoLimpio = ` ${this.normalizarTextoIA(texto).replace(/[^a-z0-9\s]/g, ' ')} `
      .replace(/\s+/g, ' ');

    return palabras.reduce((total, palabra) => {
      const termino = this.normalizarTextoIA(palabra).replace(/[^a-z0-9\s]/g, ' ').trim();
      ...
      const patron = new RegExp(`(^|\\s)${terminoEscapado}(?=\\s|$)`, 'i');

      return patron.test(textoLimpio) ? total + 1 : total;
    }, 0);
  }
  ...
  async chatAsistenteIA(preguntaOriginal: string, historial: any[] = []) {
    ...
    if (temasDetectados.includes('ventas')) {
      const resumenVentas = await this.consultaSeguraIA(`
        SELECT
          COUNT(*)::int AS total_ventas,
          COALESCE(SUM(total), 0)::numeric(12,2) AS total_vendido,
          COALESCE(ROUND(AVG(total), 2), 0)::numeric(12,2) AS ticket_promedio
        FROM app.ventas
      `);

      const ventasPorSucursal = await this.consultaSeguraIA(`
        SELECT
          s.nombre AS sucursal,
      ...
      const resumen = resumenVentas[0] || {};
      const mejorSucursal = ventasPorSucursal[0];
      const detalleSucursales = ventasPorSucursal.length
        ? ventasPorSucursal
            .map(
              (item: any, index: number) =>
                `${index + 1}. ${item.sucursal}: ${item.cantidad_ventas} ventas, ${this.formatearBsIA(item.total_vendido)}.`,
            )
            .join('\n')
        : 'Sin ventas por sucursal registradas.';

      respuestas.push(
        `Análisis de ventas:

` +
          `- Ventas registradas: ${resumen.total_ventas || 0}.
` +
          `- Total comercial vendido: ${this.formatearBsIA(resumen.total_vendido || 0)}.
` +
          `- Ticket promedio: ${this.formatearBsIA(resumen.ticket_promedio || 0)}.
` +
...
` +
    ...
    if (temasDetectados.includes('backups')) {
      ...
      const configAuto = await this.consultaSeguraIA(`
        SELECT
          activo,
          frecuencia_horas,
          ultima_ejecucion,
      ...
      const config = configAuto[0] || null;
      const lineaAutomatica = config
        ? `- Backups automáticos: ${config.activo ? 'activos' : 'desactivados'}, frecuencia cada ${config.frecuencia_horas} hora(s).\n` +
          `- Última ejecución: ${config.ultima_ejecucion || 'sin ejecución registrada'}.\n` +
          `- Próxima ejecución: ${config.proxima_ejecucion || 'sin programación activa'}.\n` +
          `- Estado de última ejecución: ${config.estado_ultima_ejecucion || 'SIN_EJECUCION'}.\n`
        : `- Backups automáticos: configuración pendiente de inicializar.\n`;
      ...
      respuestas.push(
        `Análisis de backups:\n\n` +
          ...
          `Recomendación IA: mantener activa la frecuencia automática y ejecutar una copia manual después de cambiar la programación.`,
      );
    }
